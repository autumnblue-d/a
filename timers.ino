...
static volatile u64 ticker = 0;

u64 get_ticks() {
  u64 t = ticker;
  // Serial.printf("number of ticks: %d\n", t);
  return t;  //DF
}
...
u8 timer_remove(softTimer_t* t) {
  ...
  if ((t->next == NULL) || (t->prev == NULL)) {
    irqs_resume(irq_flags);
    // print_dbg(" ; timer was not linked; returning");
    return 0;
  }
  ...
  if (found) {
    // print_dbg(" ; timer was found; unlinking");
...
void process_timers(void) {
  ...
  for (i = 0; i < num; ++i) {
    ...
    // save next, because the callback could remove t from the timer list
    volatile softTimer_t* next = t->next;
